  ...
  return (
    <Section id={sectionIds.differentials} className="section-divider bg-paper">
      <div className="grid gap-12 lg:grid-cols-[0.82fr_1.18fr] lg:gap-16">
        <div className="cmyk-stripe relative overflow-hidden rounded-[2rem] border border-black/10 bg-ink px-6 py-8 text-white shadow-panel sm:px-8">
          <div className="bg-technical-grid absolute inset-0 opacity-[0.08]" aria-hidden="true" />
          <div className="relative">
            <p className="text-[0.72rem] font-semibold uppercase tracking-[0.28em] text-white/55">Diferenciais</p>
            <h2 className="mt-5 max-w-md font-display text-[2.4rem] font-semibold leading-tight text-white">
            ...
            <div className="mt-8">
              <Button variant="secondary" onClick={scrollToSection(`#${sectionIds.contact}`)} className="border-white/20 bg-white text-ink">
                Solicitar orçamento
              </Button>
            </div>
          </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {differentials.map((item, index) => (
            <motion.article
              ...
              className="rounded-[1.5rem] border border-black/10 bg-white px-5 py-5"
            >
              <p className="text-[0.72rem] font-semibold uppercase tracking-[0.24em] text-muted">0{index + 1}</p>
